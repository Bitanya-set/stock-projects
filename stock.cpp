...
const string admin_password = "dsa";
...
void sortCategories() {
    ...
    for (Category* i = start; i != NULL && i->next != NULL; i = i->next) {
        for (Category* j = i->next; j != NULL; j = j->next) {
            if (i->name > j->name) {
                swap(i->name, j->name);
...
void sortProducts(Category* category) {
    for (int i = 1; i < category->productCount; ++i) {
        Product max = category->products[i];
        int j = i - 1;
        while (j >= 0 && category->products[j].name > max.name) {
            category->products[j + 1] = category->products[j];
            j--;
        }
        category->products[j + 1] = max;
    }
}
...
void updateProduct(const string categoryName) {
    Category* category = findCategory(categoryName);
    if (!category) {
        cout << "Incorrect category name.\n";
        return;
   ...
   int productId;
                cout << "Enter product ID to update: "; cin >> productId;
    for (int i = 0; i < category->productCount; ++i) {
...
void removeProduct(const string categoryName) {
    Category* category = findCategory(categoryName);
    if (category == NULL) {
        cout << "Incorrect category name.\n";
        return;
    ...
    for (int i = 0; i < category->productCount; ++i) {
        if (category->products[i].id == productId) {
            ...
            //int quantity;
            cout << "Enter quantity to remove: "; cin >> quantity;
            if(category->products[i].quantity < quantity){
...
int binarySearchProduct(Category* category, int productId) {
    int bott = 0, top = category->productCount - 1;
    while (bott <= top) {
        int mid = (bott + top) / 2;
        if (category->products[mid].id == productId) return mid;
        else if (category->products[mid].id < productId) bott= mid + 1;
        else top = mid - 1;
    }
    return -1;
}
...
void sortMenu() {
    ...
    cout << "2. Sort products (by name - insertion sort)\n";
